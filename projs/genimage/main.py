...
def get_date_desc():
    ...
    wd = ["一","二","三","四","五","六","日"]
    str_day = "%d年%d月%d日 %d:%d:%d 星期%s" % (to.tm_year, to.tm_mon, to.tm_mday, to.tm_hour, to.tm_min, to.tm_sec, wd[to.tm_wday])
    return str_day

def draw_img(file_name, day_comment, title, cont):
...
class LSJHandler(tornado.web.RequestHandler):
    def post(self):
        ...
        cont = self.get_argument("cont")
        #self.render("user.html",username=user_name,email=user_email,website=user_website,language=user_language)
        draw_img("LSJ.jpg", day_comment, title, cont)
        data = open('xx.jpg').read()
        self.write(data)
        self.set_header("Content-type", "image/jpg")
    def get(self):
