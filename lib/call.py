...
def build_fake_res_key(dst_url, params):
    """
    构造fake response 的key
    :return: 
    """
    params_str = ujson.dumps(params, ensure_ascii=False)
    if isinstance(params_str, unicode):
        params_str = params_str.encode('utf-8')
    if isinstance(dst_url, unicode):
        dst_url = dst_url.encode('utf-8')
    return '_'.join([dst_url, params_str])
...
def async_post(url, params=None, retjson=True, retcodesuccess=True, data=None, method='POST', headers=None,
    ...
    # 单元测试环境下 fake response 数据
    from config import config
    if getattr(config, 'IS_TEST_CASE', False):
        fake_key = build_fake_res_key(url, params)
        res = FAKE_HTTP_RES_MAP.get(fake_key, {})
        log.debug('async_post fake_key: %s' % fake_key)
